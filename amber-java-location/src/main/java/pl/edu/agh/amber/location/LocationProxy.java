...
public class LocationProxy extends AmberProxy {
	 ...
	 * 
	 * public void subscribe(CyclicDataListener<String> listener) throws
	 * IOException { logger.fine(String.format("Registering listener"));
	 * 
	 * CommonProto.DriverMsg driverMsg = buildSubscribeActionMsg();
	 * 
	 * synchronized (listenerLock) { stringListener = listener; }
	 * 
	 * amberClient.sendMessage(buildHeader(), driverMsg); }
	 * 
	 * private CommonProto.DriverMsg buildSubscribeActionMsg() {
	 * CommonProto.DriverMsg.Builder driverMsgBuilder =
	 * CommonProto.DriverMsg.newBuilder();
	 * driverMsgBuilder.setType(CommonProto.DriverMsg.MsgType.SUBSCRIBE); return
	 * driverMsgBuilder.build(); }
	 * 
	 * public void unsubscribe() throws IOException { CommonProto.DriverMsg
	 * driverMsg = buildUnsubscribeActionMsg();
	 * 
	 * synchronized (listenerLock) { stringListener = null; }
	 * 
	 * amberClient.sendMessage(buildHeader(), driverMsg); }
	 * 
	 ...
	 * return driverMsgBuilder.build(); }
	 * 
	 * public Status getStatus() throws IOException { logger.fine("Get status");
	 * 
	 * int synNum = getNextSynNum();
	 * 
	 * CommonProto.DriverMsg msg = buildGetStatusRequestMsg(synNum);
	 * 
	 * Status status = new Status(); futureObjectsMap.put(synNum, status);
	 * 
	 * amberClient.sendMessage(buildHeader(), msg);
	 * 
	 * return status; }
